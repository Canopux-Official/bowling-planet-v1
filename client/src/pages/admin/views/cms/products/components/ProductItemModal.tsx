...
export const ProductItemModal: React.FC<Props> = ({ item, baseProductId, isOpen, onClose, onSaveSuccess }) => {
  ...
  const [points, setPoints] = useState<IFeaturePoint[]>([]);
  const [usedIn, setUsedIn] = useState<IUsageLocation[]>([]);
  // New pending image files per usedIn location, keyed by array index.
  // Index alignment relies on usedIn entries never being reordered, only
  // appended/removed — same assumption the rest of this form already makes.
  const [newUsedInFiles, setNewUsedInFiles] = useState<File[][]>([]);
  ...
  useEffect(() => {
    if (item) {
      ...
      setPoints(item.points || []);
      setUsedIn(item.usedIn || []);
      setNewUsedInFiles((item.usedIn || []).map(() => []));
    } else {
      setTitle(''); setDescription(''); setStatus('active'); setPrice(''); setFeaturedOrder('0');
      setExistingThumbnailUrl(null); setExistingGallery([]);
      setFeatureList([]); setPoints([]); setUsedIn([]);
  ...
  // --- Used In ---
  const addUsedIn = () => {
    setUsedIn([...usedIn, { name: '', description: '', images: [] }]);
    setNewUsedInFiles([...newUsedInFiles, []]);
  };
  const updateUsedIn = (idx: number, key: keyof IUsageLocation, val: string) => {
    const u = [...usedIn]; u[idx] = { ...u[idx], [key]: val }; setUsedIn(u);
  };
  const removeUsedIn = (idx: number) => {
    setUsedIn(usedIn.filter((_, i) => i !== idx));
    setNewUsedInFiles(newUsedInFiles.filter((_, i) => i !== idx));
  };

  // Existing (already-uploaded) images on a usedIn location
  const removeExistingUsedInImage = (locIdx: number, imgIdx: number) => {
    const u = [...usedIn];
    u[locIdx] = {
      ...u[locIdx],
      images: (u[locIdx].images || []).filter((_, i) => i !== imgIdx),
    };
    setUsedIn(u);
  };

  // New (pending upload) images on a usedIn location
  const handleUsedInImageChange = (locIdx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files) return;
    const files = Array.from(e.target.files);
    const u = [...newUsedInFiles];
    u[locIdx] = [...(u[locIdx] || []), ...files];
    setNewUsedInFiles(u);
  };
  const removeNewUsedInImage = (locIdx: number, fileIdx: number) => {
    const u = [...newUsedInFiles];
    u[locIdx] = u[locIdx].filter((_, i) => i !== fileIdx);
    setNewUsedInFiles(u);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Partial<IProductItem> = {
      ...
      featureList,
      points,
      // usedIn already carries each location's *existing* images (edited
      // in place above). New pending files live in newUsedInFiles and
      // still need to be uploaded and merged in by the service layer —
      // see TODO below.
      usedIn,
    };

    try {
      // TODO: productItemService.create/update currently only accept
      // (payload, thumbnailFile, galleryFiles). To persist newly added
      // usedIn images, this call needs to also send `newUsedInFiles`
      // (File[] per location index) so the backend can upload them and
      // append the resulting IMedia entries to the right usedIn[i].images.
      // Wiring this depends on how the service/endpoint expects multipart
      // fields to be shaped — share that file and I'll finish this part.
      if (item) {
        await productItemService.update(item._id, payload, thumbnailFile, newGalleryFiles, newUsedInFiles);
        showToast('success', 'Product item updated successfully');
      } else {
        await productItemService.create(payload, thumbnailFile as File, newGalleryFiles, newUsedInFiles);
        showToast('success', 'Product item created successfully');
      }
      onSaveSuccess();
      onClose();
            ...
            {usedIn.map((loc, idx) => (
              <div key={idx} style={subCardStyle}>
                ...
                <div style={{ paddingLeft: '4px' }}>
                  <label style={{ ...labelStyle, fontSize: '12px', color: theme.colors.adminTextMuted }}>Images</label>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
                    {(loc.images || []).map((m, imgIdx) => (
                      <div key={imgIdx} style={thumbnailWrapperStyle}>
                        <img src={m.url} alt="usedIn" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        <button type="button" onClick={() => removeExistingUsedInImage(idx, imgIdx)} style={deleteThumbBtnStyle}><Trash2 size={12} /></button>
                      </div>
                    ))}
                    {(newUsedInFiles[idx] || []).map((f, fileIdx) => (
                      <div key={fileIdx} style={{ ...thumbnailWrapperStyle, opacity: 0.7, backgroundColor: theme.colors.adminBorder }}>
                        <div style={{ fontSize: '10px', padding: '4px' }}>{f.name}</div>
                        <button type="button" onClick={() => removeNewUsedInImage(idx, fileIdx)} style={deleteThumbBtnStyle}><Trash2 size={12} /></button>
                      </div>
                    ))}
                  </div>
                  <label style={{ ...uploadAreaStyle, padding: '10px' }}>
                    <Upload size={14} /> Add Images
                    <input type="file" multiple accept="image/*" onChange={(e) => handleUsedInImageChange(idx, e)} style={{ display: 'none' }} />
                  </label>
                </div>
              </div>
            ))}
