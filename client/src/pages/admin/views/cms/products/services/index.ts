...
function buildProductItemFormData(
    ...
    if (data.featureList) fd.append('featureList', JSON.stringify(data.featureList));
    if (data.points) fd.append('points', JSON.stringify(data.points));
    // usedIn JSON already carries each location's *remaining existing*
    // images (deletions made in the modal are reflected here) — same
    // pattern as gallery below. New files per location are appended
    // separately, tagged with their location index.
    if (data.usedIn) fd.append('usedIn', JSON.stringify(data.usedIn));
    if (data.gallery) fd.append('gallery', JSON.stringify(data.gallery)); // existing gallery kept on update
    if (thumbnailFile) fd.append('thumbnail', thumbnailFile);
    galleryFiles.forEach((f) => fd.append('gallery', f));

    // New usedIn images, one FormData field per location index so the
    // backend can tell which usedIn[] entry each file belongs to.
    usedInFiles.forEach((files, locIdx) => {
        files.forEach((f) => fd.append(`usedInImages_${locIdx}`, f));
    });
...
export const productItemService = {
    ...
    create: async (
        ...
        const body = buildProductItemFormData(data, thumbnailFile, galleryFiles, usedInFiles);
        return apiClient('/product-items/admin', {
            method: 'POST',
            headers: { 'Content-Type': undefined as any },
            body,
